...
export default function useDonateList(isPassed: boolean) {
  ...
  const {
    data: donateList,
    isSuccess: getDonateListIsSuccess,
    isLoading,
  } = useContractRead({
    ...charityContract,
  ...
  const donates: DonateWithPerson[] = getDonateListIsSuccess
    ? donateList
        .filter((donateWithPerson) => donateWithPerson.donate.isPassed === isPassed)
        .map((subDonateWithPerson) => ({
